...
module TurboPower
  module StreamHelper
    class ReplaceStateTest < StreamHelperTestCase
      ...
      test "replace_state with state as kwargs" do
        stream = %(<turbo-stream action="replace_state" url="/users/1" title="User 1" state="user-1"><template></template></turbo-stream>)

        assert_dom_equal stream, turbo_stream.replace_state("/users/1", "User 1", state: "user-1")
      end

      test "replace_state with title and state as kwargs" do
        stream = %(<turbo-stream action="replace_state" url="/users/1" title="User 1" state="user-1"><template></template></turbo-stream>)
      ...
      test "replace_state with state, url and title as kwargs" do
        stream = %(<turbo-stream action="replace_state" url="/users/1" title="User 1" state="user-1"><template></template></turbo-stream>)

        assert_dom_equal stream, turbo_stream.replace_state(state: "user-1", url: "/users/1", title: "User 1")
      end
